"""
This script will be used for training the CNN. The only input argument it
should receive is the path to the configuration file in which I define all
...
def main(config_file):
    ...
    if len(class_names) != config["model"]["classes"]:
        raise ValueError(
            "The number classes between your dataset and the model"
            "doen't match."
        )

    # Load training dataset
    # Split train data in train/validation while training the
    # model, keeping away from the experiments the testing dataset
