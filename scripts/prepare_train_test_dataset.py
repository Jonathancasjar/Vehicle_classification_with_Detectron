...
def main(data_folder, labels, output_data_folder):
    ...
    if not os.path.exists(output_data_folder):
        ...
        for subset in subsets:
            ...
            os.makedirs(path)
            for name in n_cars:
                path2 = os.path.join(output_data_folder,subset,name)
                os.makedirs(path2)
    #   3. Copy the image to the new folder structure.
        for i in range(len(df)):
            src = data_folder+str(df.loc[i, 'img_name'])
            dst = (output_data_folder+str(df.loc[i, 'subset'])+'/'+str(df.loc[i, 'class'])+'/'+str(df.loc[i, 'img_name']))
